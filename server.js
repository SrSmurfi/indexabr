...
// Endpoint para servir o manifest
app.get('/manifest.json', (req, res) => {
  res.sendFile(__dirname + '/manifest.json');
});

// Endpoint principal - usado pelo Stremio
app.get('/:id/stream/:type/:imdb.json', async (req, res) => {
  try {
    const { id, type, imdb } = req.params;
    
    console.log(`[Stremio] Buscando: type=${type} imdb=${imdb} id=${id}`);
    
    // Extrair o ID real do IMDB (ttXXXXXX)
    const imdbId = imdb.split(':')[0];
    
    // Buscar streams - aqui você deve implementar sua lógica de scraping
    // Por enquanto, vou retornar um exemplo com links de teste
    const streams = [];
    
    // Exemplo de stream (substitua com sua fonte real)
    streams.push({
      name: 'IndexaBR',
      title: `Stream ${type} - ${imdbId}`,
      url: `https://example.com/stream/${imdbId}`
    });
    
    // Resposta no formato esperado pelo Stremio
    res.json({
      streams: streams
    });
    ...
    res.status(500).json({ 
      streams: [],
      error: 'Erro interno ao buscar streams'
    });
  }
});
